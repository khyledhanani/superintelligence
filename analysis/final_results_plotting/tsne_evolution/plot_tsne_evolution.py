"""t-SNE evolution at selected training timesteps (perplexity=40).

Plots buffer embeddings at explicitly specified timesteps using
solved-rollout-only embeddings:
...
Usage:
    python analysis/final_results_plotting/plot_tsne_evolution.py \
...
def main():
    ...
    parser.add_argument("--timesteps", type=int, nargs='+', required=True,
                        help="Explicit list of timesteps to plot (e.g. 2250 2500 3000)")
    ...
    if args.output is None:
        args.output = (f"analysis/final_results_plotting/tsne_evolution/"
                       f"tsne_evolution_seed{args.seed}.png")
    ...
    # Determine which ancestor_ids are NEW at the second timestep (the injection point)
    inject_ts = timesteps[1] if len(timesteps) > 1 else timesteps[0]
    pre_ancs = set()
    pre_ts = timesteps[0]
    if pre_ts in data:
        pre_mask = data[pre_ts]["origins"] > 0
        if pre_mask.any():
            pre_ancs = set(data[pre_ts]["ancestor_ids"][pre_mask].tolist())
    pre_ancs.discard(-1)
    ...
    for panel_idx, ts in enumerate(timesteps):
        ...
        subtitle = f"({n_new_llm} tracked / {len(origins)} buffer, {n_solved_eval}/8 eval-13, {n_solved_eval21}/8 eval-21)"
        ax.set_title(f"t={ts}\n{subtitle}", fontsize=8)
        ax.set_xticks([]); ax.set_yticks([])
    ...
    legend_els = [
        ...
        Line2D([0], [0], marker='o', color='w', markerfacecolor='tab:orange',
               markersize=6, label='LLM descendant'),
    ...
    fig.legend(handles=legend_els, fontsize=8, loc='lower center',
               ...
               bbox_to_anchor=(0.5, -0.04), columnspacing=1.2,
               handletextpad=0.4)

    plt.suptitle(f"t-SNE LLM Maze Provenance",
                 fontsize=14, y=1.01)
    plt.tight_layout()
