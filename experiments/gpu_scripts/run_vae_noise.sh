...
# more updates.
#
# Usage: bash experiments/gpu_scripts/run_vae_noise.sh [target_eligible] [inject_pcts] [sigma] [seeds...]
# Example: bash experiments/gpu_scripts/run_vae_noise.sh 1000 5,10,15,20,25 0.5 0 1 2
...
TRAIN_UPDATES=10000
WANDB_GROUP="vaenoise_s${SIGMA}_t${TARGET_ELIGIBLE}"
...
echo ""
echo "=== VAE Noise Mutation ==="
echo "  target_eligible=$TARGET_ELIGIBLE  inject_pcts=$INJECT_PCTS  sigma=$SIGMA"
echo "  seeds=${SEEDS[*]}  train_updates=$TRAIN_UPDATES"

for SEED in "${SEEDS[@]}"; do
    RUN_NAME="vae_noise_s${SIGMA}_t${TARGET_ELIGIBLE}_seed${SEED}"
    ...
    for PCT_FILE in "$OUTPUT_DIR"/merged_buffer_*pct.npz; do
        ...
        TRAIN_RUN="inject_vaenoise_s${SIGMA}_${PCT_TAG}_seed${SEED}"
