#!/bin/bash
# ACCEL with uniform wall count DR (0-25 walls) on 13x13.
# Matches the ACCEL paper's DR setup where levels start simple.
# 3 seeds sequential.
...
RUN=accel_uniformdr_sfl_13x13_v1
...
for SEED in "${SEEDS[@]}"; do
    echo ""
    echo "=== ACCEL + uniform DR 13x13 seed $SEED ==="
    $PY $SCRIPT \
        --maze_height 13 --maze_width 13 --n_walls 50 \
        --use_accel \
        --uniform_wall_count \
...
echo ""
echo "ACCEL + uniform DR 13x13 done."
