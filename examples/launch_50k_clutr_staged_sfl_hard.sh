...
# 3 seeds x 50k updates, WandB project JAXUED_LEARNABILITY, group clutr-staged-sfl-hard
set -e
...
for seed in 0 1 2; do
  ...
  $PYTHON examples/maze_plr.py $COMMON $EVAL_LEVELS \
    ...
    --score_function sfl \
    --seed $seed \
    --run_name "clutr-staged-sfl-hard" \
    2>&1 | tee logs/50k_clutr_staged_sfl_hard_seed${seed}.log

  echo "=== [$(date)] Seed $seed complete ==="
done

echo "All CluttrVAE staged + SFL (hard eval) experiments complete."
