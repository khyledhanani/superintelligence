#!/bin/bash
# 50k Experiment: CNN-VAE (old 200k) + staged PCA-CMA-ES + ACCEL + SFL learnability
# Same staged params as pca-staged-accel, but with SFL scoring instead of MaxMC
# 3 seeds x 50k updates, WandB project JAXUED_50K, group cnn-staged-sfl
set -e
...
for seed in 0 1 2; do
  ...
  $PYTHON examples/maze_plr.py $COMMON \
    ...
    --score_function sfl \
    --seed $seed \
    --run_name "cnn-staged-sfl" \
    2>&1 | tee logs/50k_cnn_staged_sfl_seed${seed}.log

  echo "=== [$(date)] Seed $seed complete ==="
done

echo "All CNN staged + SFL experiments complete."
