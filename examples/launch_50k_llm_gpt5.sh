#!/bin/bash
# 50k Experiment: ACCEL + LLM injection (OpenRouter GPT-5, feedback mode)
# Based on v3 settings (buffer_mean/buffer_median gates) with new prompt
...
OUTDIR="$(cd "$(dirname "$0")/.." && pwd)/results/accel-llm-gpt-v6"
mkdir -p "$OUTDIR"
...
seed=0
echo "=== [$(date)] LLM+ACCEL (GPT-5) seed $seed starting ==="

$PYTHON examples/maze_plr.py $COMMON $LLM \
  --use_accel \
  --seed $seed \
  --run_name "llm-accel-gpt5" \
  --output_dir "$OUTDIR" \
  2>&1 | tee logs/50k_llm_gpt5_seed${seed}.log
