#!/bin/bash
# 50k Experiment: PCA-CMA-ES with refit every 5k + ACCEL mutation
# 3 seeds x 50k updates, WandB project JAXUED_50K, group pca-cmaes-refit5k-accel
set -e
...
for seed in 0 1 2; do
  ...
  $PYTHON examples/maze_plr.py $COMMON \
    ...
    --seed $seed \
    --run_name "pca-cmaes-refit5k-accel" \
    2>&1 | tee logs/50k_pca_refit_accel_seed${seed}.log

  echo "=== [$(date)] Seed $seed complete ==="
done

echo "All PCA-CMA-ES refit+ACCEL experiments complete."
